...
public class AdCampaignBoost {
    ...
    public static int consecutiveGoodDaysForAdCampaign(int[] projectedSales, int k) {
        int l = 0, r = 0;
        int maxDays = 0, badDays = 0;
        while (r < projectedSales.length) {
            if (isGoodDay(projectedSales[r]) || badDays < k) {
                if (!isGoodDay(projectedSales[r])) {
                    badDays++;
                }
                r++;
                maxDays = Math.max(maxDays, r - l);
            } else {
                if (!isGoodDay(projectedSales[l])) {
                    badDays--;
                }
                l++;
            }
