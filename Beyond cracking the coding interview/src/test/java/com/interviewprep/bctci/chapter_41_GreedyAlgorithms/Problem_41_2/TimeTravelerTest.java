...
class TimeTravelerTest {
    ...
    public void timeTravelerCanReachDestinationTest() {
        ...
        int k = 0;
        int maxAging = 3;
        Assertions.assertFalse(TimeTraveler.canReachDestinationWithHeap(jumpingPoints, k, maxAging));
        Assertions.assertFalse(TimeTraveler.canReachDestination(jumpingPoints, k, maxAging));

        jumpingPoints = new int[]{2020, 2024};
        k = 1;
        maxAging = 1;
        Assertions.assertTrue(TimeTraveler.canReachDestinationWithHeap(jumpingPoints, k, maxAging));
